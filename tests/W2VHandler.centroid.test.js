...
const { default: W2VHandler } = require('../lib/word_processor/W2VHandler')
...
`
async function main() {
    const w2v = await W2VHandler.getInstance()
    const centroid_out = w2v.centroid_out(testText)
    const centroid_in = w2v.centroid_in(testText)
    return centroid_in.dot(centroid_out)
    // tf.print(centroid)
}
